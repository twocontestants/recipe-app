...
When the cook holds a dinner, the right-hand rail shows **Earlier**, a short run of nearby days centred on the dinner’s date, and **Later**. On a typical phone that run is the dinner’s day plus two days before and two after. On a taller screen more days appear, still centred on the dinner, so Later is never clipped off the bottom.

**Why this priority**: Later is unusable when the rail is too long. Nearby days matter more than filling the strip.

**Independent Test**: Hold a dinner on a short viewport — five numbered days plus Earlier and Later are fully visible. Repeat on a tall viewport — more numbered days appear and Later is still fully visible.
...
4. **Given** they drop on a numbered day, Earlier, or Later, **When** the drag ends, **Then** existing move behaviour is unchanged.
...
- Very short viewports still show at least origin ±2 plus Earlier and Later (they may be compact, but Later remains reachable).
...
- **Rail window**: Origin date, count of numbered days, Earlier/Later slots, viewport height used to choose the count.
...
- **SC-001**: On a phone-height screen, Later is fully visible during a hold-drag (no clipping).
...
- Typical phone height is a kitchen phone in portrait (~600–750px of usable rail height).
