    ...
    <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal planner-quick-modal" role="dialog" aria-labelledby="pqm-title">
        <div className="modal-header pqm-header">
          <div>
            <h2 id="pqm-title" className="modal-title pqm-title">Add to planner</h2>
        ...
        >
          {adding
            ...
            : <>Add dinner · {DAY_SHORT[selectedKey]} {selectedDate.getDate()}</>}
        </button>
      </div>

      <style>{addToPlannerModalCss}</style>
    </div>
  );
}

export const addToPlannerModalCss = `
  .planner-quick-modal {
    max-width: 420px;
